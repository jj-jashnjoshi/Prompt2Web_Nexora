...
export default function Cart() {
  ...
  if (paid) {
    return (
      <div className="relative min-h-dvh overflow-hidden bg-red text-white flex flex-col items-center justify-center px-5 text-center">
        ...
        <Cloud className="relative mt-6 size-64 animate-pop" fill="fill-white" bumps={14}>
  ...
  const payBar = (
    <form onSubmit={checkout} className="rounded-[2rem] bg-neutral-900 p-3 text-white shadow-[0_6px_0_0_rgba(74,28,19,0.2)]">
  ...
  return (
    <Layout s={s} bottom={payBar}>
      ...
      <div className={`relative mt-4 overflow-hidden rounded-[2rem] p-5 text-white animate-rise ${makesBell ? 'bg-blue' : 'bg-red'}`}>
      ...
      <ul className="mt-4 space-y-3 pb-20">
        {lines.map((l, i) => (
          <li key={l.id} style={{ animationDelay: `${i * 60}ms` }} className="rounded-[2rem] bg-neutral-50 p-3 animate-rise">
            <div className="flex items-center gap-3">
              <span className="grid size-16 shrink-0 place-items-center rounded-3xl bg-white">
                <FoodIcon id={l.id} className="size-12" />
              </span>
            ...
            {l.eta > s.minsLeft && (
              <div className="relative mt-3 overflow-hidden rounded-3xl bg-amber p-4 text-neutral-900">
                <Squiggle className="text-white/30" />
                <p className="relative text-sm">
                  <span className="font-bold">{STATIONS[l.station].label} is jammed.</span> This lands after the bell.
                </p>
                {l.swap && (
                  <div className="relative mt-3 flex items-center gap-3 rounded-full bg-white p-1.5 pl-2">
                    <FoodIcon id={l.swap.id} className="size-9 shrink-0" />
                    ...
                    <button
                      ...
                      className="h-10 px-5 rounded-full bg-red text-white font-display font-bold shrink-0 animate-wiggle active:scale-95"
                    >
                      Swap
                    </button>
                  </div>
